        ...
        <div className="m-5 lg:m-10 lg:p-8 grid grid-cols md:grid-cols-3 gap-6 py-6 ">
          {categories.slice(0,3).map((item, index) => (
            <div
              ...
              className="bg-orange-200 cursor-pointer rounded-lg p-4 flex justify-between items-center border-orange-300 border-2 shadow-2xl shadow-orange-200 hover:scale-105 duration-500 lg:flex-col lg:h-[200px] lg:justify-center lg:items-center"
            >
              <h2 className="font-bold sm:text-xl">{item.name}</h2>
              <img src={item.image} alt={item.name} className="w-20 hover:scale-40" />
            </div>
          ))}
        </div>
      </div>
